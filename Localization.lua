...
L[ [=[Behave as if the interface option "Auto self cast" was enabled, e.g. look for friendly auras on yourself when you are not targeting a friendly unit.
Note: this enables the old Inline Aura behavior with friendly spells.]=] ] = [=[Fait comme si l'option "Soi-même par défaut" était activée, e.g. recherche les auras sur vous-même quand vous ne ciblez pas un allié.
Note: cela active l'ancien comportement d'Inline Aura avec les sorts bénéfiques.]=]
...
L["Watch aura changes on the unit under the mouse cursor. Required only to properly update macros that uses @mouseover targeting."] = "Surveille les changements d'auras de l'unité sous le curseur de la souris. Requis uniquement pour gérer correctement les macros qui utilise @mouseover."
L["Watch aura changes on your focus. Required only to properly update macros that uses @focus targeting."] = "Surveille les changements d'auras de la focalisation. Requis uniquement pour gérer correctement les macros qui utilise @focus."
...
L[ [=[Behave as if the interface option "Auto self cast" was enabled, e.g. look for friendly auras on yourself when you are not targeting a friendly unit.
Note: this enables the old Inline Aura behavior with friendly spells.]=] ] = [=[So verhalten als ob die Interface option "autom. Selbstzauber" eingeschaltet wäre. (Nach unterstüzende Auren auf dir selbst schauen wenn du einen Gener als Angriffsziel hast hast.)
Notiz: diese Option aktiviert den ehemaligen umgang von Inline Aura mit unterstüzenden Zaubern.]=]
...
L["Watch aura changes on the unit under the mouse cursor. Required only to properly update macros that uses @mouseover targeting."] = "Auf Auren wechsel bei Mouseover Zielen reagieren. Erfordert das erneuern von Macros die @mouseover benutzen."
L["Watch aura changes on your focus. Required only to properly update macros that uses @focus targeting."] = "Beacht Auren veränderungen auf deinem fokus target. Erfordert das erneuern von Macros die @focus benutzen."
...
L["COMBO_POINTS"] = "連擊點數"
L["Center"] = "中央"
L["Check to have a more accurate countdown display instead of default Blizzard rounding."] = "檢查有更精確的倒計時顯示，而不是默認的暴雪四捨五入。"
...
L["Check to only show aura you applied. Uncheck to always show aura, even when applied by others. Leave grayed to use default settings."] = "檢查只顯示您套用的光環。不檢查總是顯示的光環，甚至是在其他人使用。給灰色使用默認設置。" -- Needs review
